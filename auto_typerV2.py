...
class AutoTyperApp:
    ...
    def build_ui(self):
        ...
        char_delay_frame = ttk.Frame(wrapper)
        char_delay_frame.pack(pady=5)

        ttk.Label(char_delay_frame, text="Character Delay (ms): ").pack(side=tk.LEFT)
        self.char_delay_entry = ttk.Entry(char_delay_frame, width=6)
        self.char_delay_entry.insert(0, "0")
        self.char_delay_entry.pack(side=tk.LEFT)
        ...
        ttk.Label(wrapper, textvariable=self.status_var, anchor="w",
                  relief="groove").pack(fill="x", pady=5)
